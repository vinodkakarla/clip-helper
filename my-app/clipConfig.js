...
var saveClipManagerChanges = function() {
  ...
  clipsData.shortcutKey = self.valueSet.shortcutKey;
...
var init = function() {
  ...
  if(!valueSet.shortcutKey) {
    valueSet.shortcutKey = 'e';
  }
...
var setHelpContent = function() {
  var helpContent = self.valueSet.helpContent;
  if (!helpContent) {
...
var shortcutConfigHelper = function() {
  $("button#shortcut-save").off("click").on("click", function() {
    var selectedShortcut = $("label.shortcut-button.active > input.shortcut-button");
    if (selectedShortcut.length == 1) {
      self.valueSet.shortcutKey = selectedShortcut[0].value;
      jetpack.write(homeDirectory + '/.clipManager', self.valueSet);
    }
  });
  var shortcutKey = self.valueSet.shortcutKey;
  if (shortcutKey && shortcutKey.length == 1) {
    $("label.shortcut-button.active").removeClass("active");
    $("label.shortcut-button > input.shortcut-button[value=" + shortcutKey + "]").parent().addClass("active");
  }
}
